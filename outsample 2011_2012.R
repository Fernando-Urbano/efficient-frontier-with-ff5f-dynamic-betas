# OUT-OF-SAMPLE - 2011-2012 IN-SAMPLE
library(tidyverse)
library(lubridate)
library(readr)
library(readxl)
...
# GARCH
garch_ged = read_excel("Returns Hat - GARCH Dist GED (2011-2012).xlsx") %>% 
  mutate(model = 'GARCH GED')
garch_sged = read_excel("Returns Hat - GARCH Dist Skew GED (2011-2012).xlsx") %>% 
  mutate(model = 'GARCH Skew GED')
garch_norm = read_excel("Returns Hat - GARCH Dist Gaussian (2011-2012).xlsx") %>% 
  mutate(model = 'GARCH Gaussian')
garch_snorm = read_excel("Returns Hat - GARCH Dist Skew Gaussian (2011-2012).xlsx") %>% 
  mutate(model = 'GARCH Skew Gaussian')

# APARCH
aparch_ged = read_excel("Returns Hat - APARCH Dist GED (2011-2012).xlsx") %>% 
  mutate(model = 'APARCH GED')
aparch_sged = read_excel("Returns Hat - APARCH Dist Skew GED (2011-2012).xlsx") %>% 
  mutate(model = 'APARCH Skew GED')
aparch_norm = read_excel("Returns Hat - APARCH Dist Gaussian (2011-2012).xlsx") %>% 
  mutate(model = 'APARCH Gaussian')
aparch_snorm = read_excel("Returns Hat - APARCH Dist Skew Gaussian (2011-2012).xlsx") %>% 
...
sample_numbers = combinations(n = 17, r = 10, repeats.allowed = FALSE, v = seq(1, 17, 1)) %>% 
  data.frame() %>% 
...
returns <- read_csv("C:/Users/Dinho Urbano/Desktop/TCC/Dados Financeiros/17 Industries.csv") %>% 
  ...
  mutate_if(is.numeric, ~ case_when(. == -99.99 ~ NA %>% as.numeric(),
                                    TRUE ~ .)) %>% 
  mutate_if(is.numeric, ~ . / 100) %>% 
  dplyr::filter(date >= "2013-01-01" & date <= "2013-06-30") %>% 
  merge(factors_df %>% dplyr::select(date, RF), by='date') %>% 
  mutate_if(is.numeric, ~ (1 + .) / (1 + RF) - 1) %>% 
  dplyr::select(-RF) %>% 
  gather(id, value, -date) %>% 
...
all_outsample_statistics %>% 
  ...
  labs(title = 'Sharpe Out-of-Sample',
       subtitle = 'In-Sample: 2011-01-01 to 2012-12-31 \nOut-of-Sample: 2013-01-01 to 2013-06-30',
       x = 'Annualized Sharpe',
       y = '') +
  theme(legend.position = 'none') +
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
ggsave("Six Month Out-of-Sample Sharpe (2011-2012).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
all_outsample_statistics %>% 
  ...
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
ggsave("Six Month Out-of-Sample Returns (2011-2012).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
model_six_month %>% writexl::write_xlsx("Six Month Out-of-Sample (2011-2012).xlsx")
...
pvalue_sharpe_statistics %>%
  writexl::write_xlsx("Six Month Sharpe Difference Statistics (2011-2012).xlsx")
...
pvalue_rt_statistics %>%
  writexl::write_xlsx("Six Month Average Return Difference Statistics (2011-2012).xlsx")
...
returns <- read_csv("C:/Users/Dinho Urbano/Desktop/TCC/Dados Financeiros/17 Industries.csv") %>% 
  ...
  mutate_if(is.numeric, ~ case_when(. == -99.99 ~ NA %>% as.numeric(),
                                    TRUE ~ .)) %>% 
  mutate_if(is.numeric, ~ . / 100) %>% 
  dplyr::filter(date >= "2013-01-01" & date <= "2013-12-31") %>% 
  merge(factors_df %>% dplyr::select(date, RF), by='date') %>% 
  mutate_if(is.numeric, ~ (1 + .) / (1 + RF) - 1) %>% 
  dplyr::select(-RF) %>% 
  gather(id, value, -date) %>% 
...
all_outsample_statistics %>% 
  ...
  theme_minimal() +
  labs(title = 'Sharpe Out-of-Sample',
       subtitle = 'In-Sample: 2011-01-01 to 2012-12-31 \nOut-of-Sample: 2013-01-01 to 2013-12-31',
       x = 'Annualized Sharpe',
       y = '') +
  theme(legend.position = 'none') +
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
ggsave("One Year Out-of-Sample Sharpe (2011-2012).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
all_outsample_statistics %>% 
  ...
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
ggsave("One Year Out-of-Sample Returns (2011-2012).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
model_one_year %>% writexl::write_xlsx("One Year Out-of-Sample (2011-2012).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
pvalue_sharpe_statistics %>%
  writexl::write_xlsx("One Year Sharpe Difference Statistics (2011-2012).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
pvalue_rt_statistics %>%
  writexl::write_xlsx("One Year Average Return Difference Statistics (2011-2012).xlsx")
...
all_outsample_statistics %>% 
  ...
  theme_minimal() +
  labs(title = 'Sharpe Out-of-Sample',
       subtitle = 'In-Sample: 2011-01-01 to 2012-12-31 \nOut-of-Sample: 2013-01-01 to 2014-12-31',
       x = 'Annualized Sharpe',
       y = '') +
  theme(legend.position = 'none') +
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
ggsave("Two Years Out-of-Sample Sharpe (2011-2012).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
all_outsample_statistics %>% 
  ...
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
ggsave("Two Years Out-of-Sample Returns (2011-2012).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
model_two_years %>% writexl::write_xlsx("Two Years Out-of-Sample (2011-2012).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
pvalue_sharpe_statistics %>%
  writexl::write_xlsx("Two Years Sharpe Difference Statistics (2011-2012).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2011-2012/Efficient Frontier")
pvalue_rt_statistics %>%
  writexl::write_xlsx("Two Years Average Return Difference Statistics (2011-2012).xlsx")
