...
# GARCH
garch_ged = read_excel("Statistics - GARCH Dist GED (2009-2010).xlsx") %>% 
  mutate(model = 'GARCH GED')
garch_sged = read_excel("Statistics - GARCH Dist Skew GED (2009-2010).xlsx") %>% 
  mutate(model = 'GARCH Skew GED')
garch_norm = read_excel("Statistics - GARCH Dist Gaussian (2009-2010).xlsx") %>% 
  mutate(model = 'GARCH Gaussian')
garch_snorm = read_excel("Statistics - GARCH Dist Skew Gaussian (2009-2010).xlsx") %>% 
  mutate(model = 'GARCH Skew Gaussian')

# APARCH
aparch_ged = read_excel("Statistics - APARCH Dist GED (2009-2010).xlsx") %>% 
  mutate(model = 'APARCH GED')
aparch_sged = read_excel("Statistics - APARCH Dist Skew GED (2009-2010).xlsx") %>% 
  mutate(model = 'APARCH Skew GED')
aparch_norm = read_excel("Statistics - APARCH Dist Gaussian (2009-2010).xlsx") %>% 
  mutate(model = 'APARCH Gaussian')
aparch_snorm = read_excel("Statistics - APARCH Dist Skew Gaussian (2009-2010).xlsx") %>% 
...
for (i in 1:length(industry_names)){
  ...
  setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2009-2010/Model Statistics")
  writexl::write_xlsx(industry_statistics, path = paste(industry_names[i], 'Statistics (2009-2010).xlsx'))
...
statistics %>% 
  ...
  labs(title = 'MSE - Dynamic Betas Adjusted Excess of Return (2009-2010)',
       x = '',
       y = 'Model')

setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2009-2010/Model Statistics")
ggsave("MSE (2009-2010).png", device = 'png', units = 'in', width = 17, height = 13)
...
statistics %>% 
  ...
  theme(legend.position = 'none',
        axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) +
  scale_x_continuous(labels = scales::scientific_format(digits = 3)) +
  labs(title = 'MAE - Dynamic Betas Adjusted Excess of Return (2009-2010)',
       x = '',
       y = 'Model')

setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2009-2010/Model Statistics")
ggsave("MAE (2009-2010).png", device = 'png', units = 'in', width = 17, height = 13)