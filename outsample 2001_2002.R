# OUT-OF-SAMPLE - 2001-2002 IN-SAMPLE
library(tidyverse)
library(lubridate)
library(readr)
library(readxl)
...
# GARCH
garch_ged = read_excel("Returns Hat - GARCH Dist GED (2001-2002).xlsx") %>% 
  mutate(model = 'GARCH GED') %>% 
  dplyr::filter(id != "Chems")
garch_sged = read_excel("Returns Hat - GARCH Dist Skew GED (2001-2002).xlsx") %>% 
  mutate(model = 'GARCH Skew GED') %>% 
  dplyr::filter(id != "Chems")
garch_norm = read_excel("Returns Hat - GARCH Dist Gaussian (2001-2002).xlsx") %>% 
  mutate(model = 'GARCH Gaussian') %>% 
  dplyr::filter(id != "Chems")
garch_snorm = read_excel("Returns Hat - GARCH Dist Skew Gaussian (2001-2002).xlsx") %>% 
  mutate(model = 'GARCH Skew Gaussian') %>% 
  dplyr::filter(id != "Chems")

# APARCH
aparch_ged = read_excel("Returns Hat - APARCH Dist GED (2001-2002).xlsx") %>% 
  mutate(model = 'APARCH GED') %>% 
  dplyr::filter(id != "Chems")
aparch_sged = read_excel("Returns Hat - APARCH Dist Skew GED (2001-2002).xlsx") %>% 
  mutate(model = 'APARCH Skew GED') %>% 
  dplyr::filter(id != "Chems")
aparch_norm = read_excel("Returns Hat - APARCH Dist Gaussian (2001-2002).xlsx") %>% 
  mutate(model = 'APARCH Gaussian') %>% 
  dplyr::filter(id != "Chems")
aparch_snorm = read_excel("Returns Hat - APARCH Dist Skew Gaussian (2001-2002).xlsx") %>% 
...
returns <- read_csv("C:/Users/Dinho Urbano/Desktop/TCC/Dados Financeiros/17 Industries.csv") %>% 
  ...
  mutate_if(is.numeric, ~ case_when(. == -99.99 ~ NA %>% as.numeric(),
                                    TRUE ~ .)) %>% 
  mutate_if(is.numeric, ~ . / 100) %>% 
  dplyr::filter(date >= "2003-01-01" & date <= "2003-06-30") %>% 
  merge(factors_df %>% dplyr::select(date, RF), by='date') %>% 
  mutate_if(is.numeric, ~ (1 + .) / (1 + RF) - 1) %>% 
  dplyr::select(-RF) %>% 
  gather(id, value, -date) %>% 
...
all_outsample_statistics %>% 
  ...
  labs(title = 'Sharpe Out-of-Sample',
       subtitle = 'In-Sample: 2001-01-01 to 2002-12-31 \nOut-of-Sample: 2003-01-01 to 2003-06-30',
       x = 'Annualized Sharpe',
       y = '') +
  theme(legend.position = 'none') +
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
ggsave("Six Month Out-of-Sample Sharpe (2001-2002).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
all_outsample_statistics %>% 
  ...
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
ggsave("Six Month Out-of-Sample Returns (2001-2002).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
model_six_month %>% writexl::write_xlsx("Six Month Out-of-Sample (2001-2002).xlsx")
...
pvalue_sharpe_statistics %>%
  writexl::write_xlsx("Six Month Sharpe Difference Statistics (2001-2002).xlsx")
...
pvalue_rt_statistics %>%
  writexl::write_xlsx("Six Month Average Return Difference Statistics (2001-2002).xlsx")
...
returns <- read_csv("C:/Users/Dinho Urbano/Desktop/TCC/Dados Financeiros/17 Industries.csv") %>% 
  ...
  mutate_if(is.numeric, ~ case_when(. == -99.99 ~ NA %>% as.numeric(),
                                    TRUE ~ .)) %>% 
  mutate_if(is.numeric, ~ . / 100) %>% 
  dplyr::filter(date >= "2003-01-01" & date <= "2003-12-31") %>% 
  merge(factors_df %>% dplyr::select(date, RF), by='date') %>% 
  mutate_if(is.numeric, ~ (1 + .) / (1 + RF) - 1) %>% 
  dplyr::select(-RF) %>% 
  gather(id, value, -date) %>% 
...
all_outsample_statistics %>% 
  ...
  theme_minimal() +
  labs(title = 'Sharpe Out-of-Sample',
       subtitle = 'In-Sample: 2000-01-01 to 2001-12-31 \nOut-of-Sample: 2003-01-01 to 2003-12-31',
       x = 'Annualized Sharpe',
       y = '') +
  theme(legend.position = 'none') +
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
ggsave("One Year Out-of-Sample Sharpe (2001-2002).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
all_outsample_statistics %>% 
  ...
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
ggsave("One Year Out-of-Sample Returns (2001-2002).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
model_one_year %>% writexl::write_xlsx("One Year Out-of-Sample (2001-2002).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
pvalue_sharpe_statistics %>%
  writexl::write_xlsx("One Year Sharpe Difference Statistics (2001-2002).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
pvalue_rt_statistics %>%
  writexl::write_xlsx("One Year Average Return Difference Statistics (2001-2002).xlsx")
...
all_outsample_statistics %>% 
  ...
  theme_minimal() +
  labs(title = 'Sharpe Out-of-Sample',
       subtitle = 'In-Sample: 2001-01-01 to 2002-12-31 \nOut-of-Sample: 2003-01-01 to 2004-12-31',
       x = 'Annualized Sharpe',
       y = '') +
  theme(legend.position = 'none') +
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
ggsave("Two Years Out-of-Sample Sharpe (2001-2002).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
all_outsample_statistics %>% 
  ...
  scale_x_continuous(labels = scales::number_format(big.mark = ',',
                                                    decimal.mark = '.'))
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
ggsave("Two Years Out-of-Sample Returns (2001-2002).png", device = 'png', units = 'in',
       width = 10, height = 7)
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
model_two_years %>% writexl::write_xlsx("Two Years Out-of-Sample (2001-2002).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
pvalue_sharpe_statistics %>% writexl::write_xlsx("Two Years Sharpe Difference Statistics (2001-2002).xlsx")
...
setwd("C:/Users/Dinho Urbano/Desktop/TCC/Fatores/2001-2002/Efficient Frontier")
pvalue_rt_statistics %>%
  writexl::write_xlsx("Two Years Average Return Difference Statistics (2001-2002).xlsx")
